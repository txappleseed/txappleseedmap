...
CSV.foreach("data/DistrictPercentage2015.csv", headers: true) do |discipline_row|
  next if discipline_row["feature"] != "E-DAEP PLACEMENTS"
  ...
  school_districts_data["features"].each do |district|
    ...
    if district_id == row_district_id
      district["properties"]["AltEdu_percent_#{group}"] = discipline_row["percentage"]
      district["properties"]["AltEdu_count_#{group}"] = discipline_row["count"]
      district["properties"]["AltEdu_scale_#{group}"] = discipline_row["scale"]
    end
  end
end
...
# export as json
File.open("geojson/altedu_districts.geojson", "w") do |file|
